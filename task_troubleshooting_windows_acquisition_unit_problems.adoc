...
| 未符合網路需求 | 確保您的採購單位伺服器 /VM 可透過連接埠 443 上的 SSL 連線、存取您的資料基礎架構洞見環境（ <environment-name> 。 NetApp .com ）。請嘗試下列命令： _ping <environment-name> <environment-name> 。 c01. cloudinsations.com_ _traceroute <environment-name> NetApp 。 c01. cloudsations.com_ _curl \https ： NetApp <environment-name> NetApp 。 c01. cloudamses.com_ _wGET \https ： NetApp 。 c01. cloudinsations.com_ 
