...
Cloud Insights 可以在收集器中選擇收集 ONTAP 叢集交換器資料 <<advanced-configuration,進階組態>> 設定：除了在 Cloud Insights 收集器上啟用此功能外、您還必須 * 設定 ONTAP 系統 * 本身以提供 link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["交換器資訊"]、並確保正確 <<a-note-about-permissions,權限>> 已設定、以便將交換器資料傳送至 Cloud Insights 。
...
| 叢集交換器度量 | 允許 Cloud Insights 收集叢集交換器資料。請注意、除了在 Cloud Insights 端啟用此功能外、您還必須設定 ONTAP 系統以提供 link:https://docs.netapp.com/us-en/ontap-cli-98/system-switch-ethernet-create.html["交換器資訊"]、並確保正確 <<a-note-about-permissions,權限>> 已設定、以便將交換器資料傳送至 Cloud Insights 。請參閱下方的「權限注意事項」。 
|===
...
NOTE: 如果未正確設定 ONTAP 存取控制、則 Cloud Insights REST 呼叫可能會失敗、導致裝置的資料缺口。  例如、如果您已在 Cloud Insights 收集器上啟用、但尚未在 ONTAP 上設定權限、則擷取將會失敗。  此外、如果先前在 ONTAP 上定義角色、而您正在新增其餘 API 功能、請確定已將 _http_ 新增至角色。
