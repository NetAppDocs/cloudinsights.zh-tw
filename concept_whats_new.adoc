...
新的工作負載安全 API 可用於管理資料收集器（新增 / 更新 / 刪除）和代理程式。瀏覽至 * 管理 > API 存取 > API 文件 * 、然後選取 _Workload Security_ API 類型以深入瞭解。
...
* 偵測演算法已增強、可減少誤報的次數。
* 在載入、顯示及捲動 Forensics 頁面時、使用者介面效能已獲得改善。
...
* * 拒絕存取 * ：工作負載安全性與 ONTAP 整合、可接收「拒絕存取」事件、並提供額外的分析和自動回應層。
