...
。 link:/task_config_telegraf_agent_k8s.html#operator-based-install-or-script-based-install["NetApp Kubernetes監控操作員"] （NKMO）適用於執行Kubernetes 1.17或更新版本的所有環境、也是安裝Kubernetes for Cloud Insights the Fesis的首選方法。NKMO比指令碼安裝的監控方法更為完善、因為它能更靈活地設定監控功能、使監控功能可從Cloud Insights 功能上控制、減少客戶導向的組態介入、並可強化監控K8s叢集中執行的其他軟體的機會。
...
您現在可以接收資料收集的警示 link:/task_system_monitors.html#collector-monitors["活動"]：
...
請注意、這些監視器預設為「暫停」、必須先啟用（恢復）、才能傳送警示。

另外請注意、收集器警告的監視器傳送的警示數量可能比其他監視器類型多、因此除非您正在疑難排解、否則建議您不要設定任何警示收件者。



==== 儲存變更記錄事件的警示

當您為Cloud Insights 由NetApp產生的儲存變更記錄事件定義監控器（使用記錄來源_logs.cloud_insights.change_log.storage_）時、會收到資訊警示、並在每次收集後比較儲存系統的過去和新狀態。下列ONTAP 事件的預設監視器可供使用以供儲存：
...
| –限制使用者在遭受攻擊時存取Cloud Secure | 偵測到攻擊時、能夠限制使用者存取、為您的業務關鍵資料提供更好的保護。您可以使用自動回應原則、或從警示或使用者詳細資料頁面手動限制存取。link:https://docs.netapp.com/us-en/cloudinsights/cs_automated_response_policies.html["瞭解更多資訊"] 
...
* *智慧型檔案*：此REST API型收集器將會取得「協同」叢集、探索「視圖」（做為CI內部磁碟區）、各種節點、並收集效能指標。
