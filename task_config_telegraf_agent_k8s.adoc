...
toc::[]
Kubernetes 運算子和資料收集器都是從 Cloud Insights Docker 登錄下載。安裝之後、操作員便會管理部署在 Kubernetes 叢集節點中的任何與操作員相容的收集器、以取得資料、包括管理這些收集器的生命週期。在這條鏈之後、資料會從收集器中擷取、並傳送至Cloud Insights
...
IMPORTANT: 閱讀 link:pre-requisites_for_k8s_operator.html["* 安裝或升級之前 *"] 安裝或升級 NetApp Kubernetes 監控操作員之前的先決條件文件。
...
| 我在 Telegraf 記錄檔中看到類似以下內容的重複錯誤訊息：
 ...
 好！[agent] 寫入 outputs.http ： POST 時發生錯誤 "https://<tenant_url>/rest/v1/lake/ingest/influxdb":[] 已超過內容期限（等待標頭時超過 Client.Timeout ） | 編輯 _AgentConfiguration_ 中的 Teledraf 區段、並將 _dockerMetricCollectionEnabled_ 設為 false 。如需詳細資訊、請參閱操作員的  link:telegraf_agent_k8s_config_options.html["組態選項"]。 
