...
| Cloud Insights 每 6 小時自動登出一次 | 這是因為瀏覽器的第三方 Cookie 已停用。使用者可以設定其瀏覽器以啟用所有第三方 Cookie 、或使用較窄的例外清單來僅啟用 Cloud Insights 的例外清單。
...
選取「允許所有Cookie」選項。
*或*
選取「封鎖第三方 Cookie 」、並新增例外情況至： __[\*.]auth0.com 和 __[*.]netapp.com 。

Microsoft Edge 提供與 Chrome 相同的例外格式。

在 Firefox 中、 Cookie 例外會指定為 Simply _auth0.com 和 _netapp.com 。 
