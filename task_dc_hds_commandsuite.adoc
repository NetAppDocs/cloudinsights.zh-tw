...
| 錯誤：*未在Storage Navigator模組化2 CLI中登錄陣列*未在Storage Navigator模組化2 CLI中登錄組態錯誤：Storage Array未在StorageNavigator模組化CLI中登錄 | * 開啟命令提示字元、並將目錄變更為設定的路徑
* 執行命令「 set=STONAVM_home= 」。
* 執行命令 "auunitref"
* 確認命令輸出包含具有 IP 的陣列詳細資料
* 如果輸出不包含陣列詳細資料、請使用 Storage Navigator CLI 登錄陣列：
    - 開啟命令提示字元、並將目錄變更為設定的路徑
    - 執行命令「 set=STONAVM_home= 」。
