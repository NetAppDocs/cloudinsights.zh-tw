...
瞭解 Cloud Insights 安全性 link:security_overview.html["請按這裡"]。
...
如需Proxy組態的詳細資訊、請參閱NetApp link:https://kb.netapp.com/Advice_and_Troubleshooting/Cloud_Services/Cloud_Insights/Where_is_the_proxy_information_saved_to_in_the_Cloud_Insights_Acquisition_Unit["知識庫"]。
...
. 在 Cloud Insights 中、移至 * 可服務性 > 收集器、然後選取 * 擷取單位 * 標籤。
...
. 登入您的 Cloud Insights 環境、前往 * 可服務性 > 收集器 * 。
