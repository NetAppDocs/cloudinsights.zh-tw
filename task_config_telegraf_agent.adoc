...
| 設定新的外掛程式並重新啟動Telegraf之後、Telegraf無法啟動。記錄顯示類似下列的錯誤：「[Telegraf]錯誤執行代理程式：載入組態檔/etc/Telegraf/Telegraf.d/cloudss-default.conf時發生錯誤：plugin outs.http：line 88：組態指定欄位["use_system_proxy"、但未使用」 | 安裝的Telegraf版本已過時。請依照本頁的步驟*升級您適當平台的代理程式*。 
